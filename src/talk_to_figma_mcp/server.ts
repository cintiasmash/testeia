...
const serverArg = args.find((arg: string) => arg.startsWith('--server='));
const serverUrl = serverArg ? serverArg.split('=')[1] : 'localhost';
const WS_URL = serverUrl === 'localhost' ? `ws://${serverUrl}` : `wss://${serverUrl}`;

// Tool reference: full command list and usage — see COMMANDS.md in project root.
...
function filterFigmaNode(node: any) {
  ...
  if (node.fillStyleId !== undefined) {
    filtered.fillStyleId = node.fillStyleId;
  }
...
// Set Fill Style (link to token) Tool
server.tool(
  "set_fill_style",
  "Link a node's fill to a design token (paint style) by style ID. Use to fix unlinked fills.",
  {
    nodeId: z.string().describe("The ID of the node to modify"),
    styleId: z.string().describe("The paint style ID from get_styles (e.g. from colors array)"),
  },
  async ({ nodeId, styleId }: any) => {
    try {
      const result = await sendCommandToFigma("set_fill_style", {
        nodeId,
        styleId,
      });
      const typedResult = result as { name: string };
      return {
        content: [
          {
            type: "text",
            text: `Linked fill of node "${typedResult.name}" to style ${styleId}`,
          },
        ],
      };
    } catch (error) {
      return {
        content: [
          {
            type: "text",
            text: `Error linking fill style: ${error instanceof Error ? error.message : String(error)}`,
          },
        ],
      };
    }
...
// Get Variables (from Variables panel - for matching colors to bind)
server.tool(
  "get_variables",
  "Get all local variables from the Figma document (id, name, resolvedType, valuesByMode). Use to match unlinked fills to Variables.",
  {},
  async () => {
    try {
      const result = await sendCommandToFigma("GET_VARIABLES_DATA") as { success?: boolean; variables?: Array<{ id: string; name: string; resolvedType: string; valuesByMode: Record<string, { r?: number; g?: number; b?: number; a?: number }> }>; variableCollections?: unknown[] };
      const vars = result?.variables ?? (result as any)?.data?.variables ?? [];
      return {
        content: [
          { type: "text" as const, text: JSON.stringify({ variables: vars, count: vars.length }) },
        ],
      };
    } catch (error) {
      return {
        content: [
          { type: "text" as const, text: `Error getting variables: ${error instanceof Error ? error.message : String(error)}` },
        ],
      };
    }
  }
...
// Set Fill Variable (bind fill to a Variable from Variables panel)
server.tool(
  "set_fill_variable",
  "Bind a node's fill to a Variable (from the Variables panel) by variable ID. Use to fix unlinked fills with Variables.",
  {
    nodeId: z.string().describe("The ID of the node to modify"),
    variableId: z.string().describe("The variable ID from get_variables (COLOR type)"),
  },
  async ({ nodeId, variableId }: any) => {
    try {
      const result = await sendCommandToFigma("set_fill_variable", { nodeId, variableId });
      const typedResult = result as { name: string };
      return {
        content: [
          { type: "text" as const, text: `Vinculado fill do nó "${typedResult.name}" à variable ${variableId}` },
        ],
      };
    } catch (error) {
      return {
        content: [
          { type: "text" as const, text: `Erro ao vincular fill à variable: ${error instanceof Error ? error.message : String(error)}` },
        ],
      };
    }
  }
...
// Get nodes with unlinked fills
server.tool(
  "get_nodes_with_unlinked_fills",
  "Get all nodes under a node (or current page) that have a fill but are not linked to a style/token.",
  {
    nodeId: z.string().optional().describe("Root node ID to scan. If omitted, uses current page."),
  },
  async ({ nodeId }: any) => {
    try {
      let rootId = nodeId;
      if (!rootId) {
        const doc = await sendCommandToFigma("get_document_info") as { currentPage?: { id: string } };
        rootId = doc?.currentPage?.id;
        if (!rootId) throw new Error("No current page and no nodeId provided");
      }
      const result = await sendCommandToFigma("get_nodes_with_unlinked_fills", { nodeId: rootId }) as { success: boolean; nodes?: Array<{ nodeId: string; name: string; fillHex: string; opacity?: number }>; error?: string };
      if (!result.success) throw new Error(result.error || "Failed to get unlinked fills");
      return {
        content: [
          { type: "text" as const, text: JSON.stringify({ nodes: result.nodes || [], count: (result.nodes || []).length }) },
        ],
      };
    } catch (error) {
      return {
...
// Link unlinked fills to tokens
server.tool(
  "link_unlinked_fills_to_tokens",
  "Find all nodes with unlinked fills under a node (or current page), match each fill to the closest design token by color, and link them. Use after selecting a frame or leave nodeId empty for whole page.",
  {
    nodeId: z.string().optional().describe("Root node ID to scan. If omitted, uses current page."),
  },
  async ({ nodeId }: any) => {
    try {
      ...
      const stylesResult = await sendCommandToFigma("get_styles") as { colors?: Array<{ id: string; name: string; paint?: { color?: { r: number; g: number; b: number }; opacity?: number } }> };
      const colors = stylesResult?.colors || [];
      const unlinkedResult = await sendCommandToFigma("get_nodes_with_unlinked_fills", { nodeId: rootId }) as { success: boolean; nodes?: Array<{ nodeId: string; name: string; fillHex: string; opacity?: number }> };
      if (!unlinkedResult.success || !unlinkedResult.nodes?.length) {
        return { content: [{ type: "text" as const, text: "Nenhum nó com preenchimento desvinculado encontrado." }] };
      }
      const nodes = unlinkedResult.nodes;
      function hexToRgb(hex: string): [number, number, number] {
        const h = hex.replace(/^#/, "");
        const r = parseInt(h.slice(0, 2), 16) / 255;
        const g = parseInt(h.slice(2, 4), 16) / 255;
        const b = h.length >= 6 ? parseInt(h.slice(4, 6), 16) / 255 : 0;
        return [r, g, b];
      }
      function colorDistance(hex: string, style: { paint?: { color?: { r: number; g: number; b: number } } }): number {
        const [r1, g1, b1] = hexToRgb(hex);
        const c = style.paint?.color;
        if (!c) return Infinity;
        return Math.sqrt((r1 - c.r) ** 2 + (g1 - c.g) ** 2 + (b1 - c.b) ** 2);
      }
      const lines: string[] = [];
      for (const node of nodes) {
        const fillHex = node.fillHex.length >= 6 ? node.fillHex.slice(0, 7) : node.fillHex;
        let best = colors.reduce<{ style: typeof colors[0]; dist: number } | null>((acc, style) => {
          const dist = colorDistance(fillHex, style);
          if (acc === null || dist < acc.dist) return { style, dist };
          return acc;
        }, null);
        if (!best || best.dist > 0.01) {
          lines.push(`Nó "${node.name}" (${node.nodeId}): cor ${fillHex} – nenhum token próximo encontrado; ignorado.`);
          continue;
        }
        const styleId = best.style.id;
        await sendCommandToFigma("set_fill_style", { nodeId: node.nodeId, styleId });
        lines.push(`Vinculado "${node.name}" ao token ${best.style.name} (${styleId}).`);
      }
      return {
        content: [
          { type: "text" as const, text: lines.length ? lines.join("\n") : "Nenhum nó atualizado." },
        ],
      };
    } catch (error) {
      return {
        content: [
          { type: "text" as const, text: `Erro ao vincular tokens: ${error instanceof Error ? error.message : String(error)}` },
        ],
      };
    }
  }
...
// Link unlinked fills to Variables (Variables panel) - preferred for design tokens in Variables
server.tool(
  "link_unlinked_fills_to_variables",
  "Find all nodes with unlinked fills, match each fill to the closest Variable (Variables panel) by color, and bind them. Use this to fix tokens that should be linked to Variables.",
  {
    nodeId: z.string().optional().describe("Root node ID to scan. If omitted, uses current page."),
  },
  async ({ nodeId }: any) => {
    try {
      ...
      const varsResult = await sendCommandToFigma("GET_VARIABLES_DATA") as { variables?: Array<{ id: string; name: string; resolvedType: string; valuesByMode: Record<string, { r?: number; g?: number; b?: number; a?: number }> }> };
      const vars = varsResult?.variables ?? (varsResult as any)?.data?.variables ?? [];
      const colorVars = vars.filter((v: any) => v.resolvedType === "COLOR");
      const unlinkedResult = await sendCommandToFigma("get_nodes_with_unlinked_fills", { nodeId: rootId }) as { success: boolean; nodes?: Array<{ nodeId: string; name: string; fillHex: string; opacity?: number }> };
      if (!unlinkedResult.success || !unlinkedResult.nodes?.length) {
        return { content: [{ type: "text" as const, text: "Nenhum nó com preenchimento desvinculado encontrado." }] };
      }
      function rgbaToHex(r: number, g: number, b: number, a?: number): string {
        const rh = Math.round(Math.max(0, Math.min(1, r)) * 255).toString(16).padStart(2, "0");
        const gh = Math.round(Math.max(0, Math.min(1, g)) * 255).toString(16).padStart(2, "0");
        const bh = Math.round(Math.max(0, Math.min(1, b)) * 255).toString(16).padStart(2, "0");
        let hex = "#" + rh + gh + bh;
        if (a !== undefined && a < 1) hex += Math.round(a * 255).toString(16).padStart(2, "0");
        return hex;
      }
      type VarEntry = { id: string; name: string; hex: string };
      const varHexList: VarEntry[] = [];
      for (const v of colorVars) {
        ...
        for (const modeId of Object.keys(modes)) {
          ...
          if (val && typeof val.r === "number" && typeof val.g === "number" && typeof val.b === "number") {
            ...
            varHexList.push({ id: v.id, name: v.name, hex });
          }
        }
      }
      function hexToRgb(hex: string): [number, number, number] {
      ...
      const lines: string[] = [];
      for (const node of unlinkedResult.nodes!) {
        const fillHex = node.fillHex.length >= 6 ? node.fillHex.slice(0, 7) : node.fillHex;
        let best: { entry: VarEntry; dist: number } | null = null;
        for (const entry of varHexList) {
          const dist = colorDistance(fillHex, entry.hex);
          if (best === null || dist < best.dist) best = { entry, dist };
        }
        if (!best || best.dist > 0.01) {
          lines.push(`Nó "${node.name}" (${node.nodeId}): cor ${fillHex} – nenhuma variable próxima; ignorado.`);
          continue;
        }
        await sendCommandToFigma("set_fill_variable", { nodeId: node.nodeId, variableId: best.entry.id });
        lines.push(`Vinculado "${node.name}" à variable ${best.entry.name} (${best.entry.id}).`);
      }
      return {
        content: [
          { type: "text" as const, text: lines.length ? lines.join("\n") : "Nenhum nó atualizado." },
      ...
      return {
        content: [
          { type: "text" as const, text: `Erro ao vincular fills às variables: ${error instanceof Error ? error.message : String(error)}` },
        ],
      };
    }
  }
...
// Run recursive token fix (broken + hardcoded + style overrides) in subtree
server.tool(
  "run_recursive_token_fix",
  "Fix all color tokens recursively in the selected node or given nodeId: broken bindings, hardcoded hex colors, and style overrides (fillStyleId/strokeStyleId). Uses a default token map (e.g. FCFCFD→background, FFFFFF→icon, 1C2024→text.primary). Prefer this when the user asks to correct or fix unlinked/broken tokens in a component. If nodeId is omitted, uses current selection or current page.",
  {
    nodeId: z.string().optional().describe("Root node ID (e.g. component or frame). If omitted, uses current selection or current page."),
  },
  async ({ nodeId }: any) => {
    try {
      const result = await sendCommandToFigma(
        ...
        60000
      ) as { success?: boolean; linked?: unknown[]; noMatch?: unknown[]; brokenFound?: number; unlinkedFound?: number; styleOverrideFound?: number; totalFixed?: number; error?: string };
      if (result.success === false) {
        throw new Error(result.error || "run_recursive_token_fix failed");
      }
      const linked = result.linked ?? [];
      const noMatch = result.noMatch ?? [];
      const totalFixed = result.totalFixed ?? linked.length;
      const text = [
        `Correção recursiva de tokens: ${totalFixed} corrigido(s).`,
        result.brokenFound != null && result.brokenFound > 0 ? `Bindings quebrados encontrados: ${result.brokenFound}.` : "",
        result.unlinkedFound != null && result.unlinkedFound > 0 ? `Cores hardcoded: ${result.unlinkedFound}.` : "",
        result.styleOverrideFound != null && result.styleOverrideFound > 0 ? `Overrides de estilo: ${result.styleOverrideFound}.` : "",
        noMatch.length > 0 ? `${noMatch.length} sem token correspondente no mapa.` : "",
      ].filter(Boolean).join(" ");
      return {
      ...
      return {
        content: [
          { type: "text" as const, text: `Erro na correção recursiva de tokens: ${error instanceof Error ? error.message : String(error)}` },
        ],
      };
    }
  }
...
server.tool(
  "rename_node",
  "Rename a node (layer/component/frame) in Figma. Organize layers and components.",
  {
    nodeId: z.string().describe("The ID of the node to rename"),
    newName: z.string().describe("The new name for the node"),
  },
  async ({ nodeId, newName }: any) => {
    try {
      const result = await sendCommandToFigma("rename_node", { nodeId, newName }) as { success?: boolean; oldName?: string; error?: string };
      const msg = result.success !== false
        ? `Renamed node to "${newName}"${result.oldName ? ` (was "${result.oldName}")` : ""}.`
        : (result.error || "Failed to rename node.");
      return {
...
// Set all TEXT layers in the document to a given string (e.g. {Text})
server.tool(
  "set_all_text_layers",
  "Set all TEXT layers in the current Figma document to a given string (e.g. {Text}). Use to normalize placeholder text across the document.",
  {
    text: z.string().describe("The text to set on every text layer").default("{Text}"),
  },
  async ({ text }: { text?: string }) => {
    try {
      const result = await sendCommandToFigma("SET_ALL_TEXT_LAYERS", { text: text ?? "{Text}" }, 120000) as { success?: boolean; total?: number; updated?: number; error?: string };
      if (result.success) {
        return {
          content: [
            { type: "text" as const, text: `Atualizadas ${result.updated ?? 0} de ${result.total ?? 0} layers de texto para "${text ?? "{Text}"}".` },
          ],
        };
      }
      return {
        content: [
          { type: "text" as const, text: `Erro: ${result.error ?? "Falha ao atualizar layers de texto"}` },
        ],
      };
    } catch (error) {
      return {
        content: [
          { type: "text" as const, text: `Erro ao definir texto em todas as layers: ${error instanceof Error ? error.message : String(error)}` },
        ],
      };
    }
  }
...
// Set Auto Layout Tool (enable Auto Layout on a frame — Flexbox-like layout)
server.tool(
  "set_auto_layout",
  "Enable or configure Auto Layout on a frame (Flexbox-like layout). Component spacing and direction.",
  {
    nodeId: z.string().describe("The ID of the frame/component to modify"),
    direction: z.enum(["HORIZONTAL", "VERTICAL"]).describe("Primary axis direction"),
    ...
    alignment: z.enum(["MIN", "CENTER", "MAX", "SPACE_BETWEEN"]).optional().describe("Primary axis alignment"),
    wrap: z.boolean().optional().describe("Allow wrapping"),
    clipContent: z.boolean().optional().describe("Clip content to bounds"),
  },
  async (params: any) => {
    try {
      const padding = [params.paddingTop, params.paddingRight, params.paddingBottom, params.paddingLeft].some((n: number | undefined) => n != null)
        ? { top: params.paddingTop, right: params.paddingRight, bottom: params.paddingBottom, left: params.paddingLeft }
        : undefined;
      ...
      return {
        content: [{ type: "text" as const, text: typeof result === "object" && result && "name" in result ? `Set auto layout on "${(result as { name: string }).name}".` : "Auto layout updated." }],
      };
    } catch (error) {
      return {
        content: [{ type: "text" as const, text: `Error setting auto layout: ${error instanceof Error ? error.message : String(error)}` }],
      };
    }
  }
);
...
type CommandParams = {
  ...
  GET_VARIABLES_DATA: Record<string, never>;
  set_fill_variable: {
    nodeId: string;
    variableId: string;
  };
...
// Reconnect attempt counter (to reduce log spam)
...
function connectToFigma(port: number = 3055) {
  ...
  ws.on('error', (error: unknown) => {
    ...
    if (reconnectAttempts === 1) {
      logger.error(`Socket error: ${full}`);
      logger.warn('Plugin Figma não está rodando. Abra o Figma, inicie o plugin "Talk to Figma" no documento e deixe-o escutando na porta 3055.');
    } else if (reconnectAttempts % 10 === 0) {
      logger.warn(`Ainda sem conexão com Figma (tentativa ${reconnectAttempts}). Aguardando plugin em ${wsUrl}...`);
    }
  });
  ...
  ws.on('close', () => {
    ...
    // Attempt to reconnect (every 10s to avoid log spam)
    const delayMs = 10000;
    if (reconnectAttempts <= 1) {
      logger.info(`Reconectando em ${delayMs / 1000} segundos...`);
    }
    setTimeout(() => connectToFigma(port), delayMs);
  });
}
